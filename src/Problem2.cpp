...
int to_left(node *, node_dll *);
int to_right(node *, node_dll *);
...
int to_left(node *curr, node_dll *head)
{
	int n,n1, n2;
	if (curr == NULL)
		return -1;
	else
	{
		...
		if (n == 0)
			return 0;
		n1 = to_left(curr->left, head);
		n2 = to_right(curr->right, head);
		if (n1 == 1 && n2 == 1)
			return 1;
		else
			return 0;
...
int to_right(node *curr, node_dll *head)
{
	int n,n1, n2;
	if (curr == NULL)
		return -1;
...
int is_identical(struct node_dll *head, struct node *root){
	...
	if (n == 0)
		return 0;
	n1=to_left(root->left, head);
	n2=to_right(root->right, head);
	if (n1 == 1 && n2 == 1)
		return 1;
	else
		return 0;
